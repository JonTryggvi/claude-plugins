...
| `setup-dev-machine` | Base tooling (Xcode CLT/git, Homebrew, `gh`), a working shell config (`~/.zprofile` + `~/.zshrc` + `~/.zsh` modules with the team functions `gsend`, `set_gh_user`, `new_ssh_key`, a `gh()` wrapper), SSH keys per GitHub account, `gh` multi-account auth, and both git identity and the active `gh` account switching themselves per project. | **Once per machine.** Also the fix when `git push` or `gh release create` hits the wrong account or returns `403`. |
