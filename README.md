...
- **Overview entry point.** Every plugin ships an `overview` skill (`skills/overview/SKILL.md`) holding a comprehensive summary — what it bundles, what each skill does, the order to use them, prerequisites — plus a thin `commands/overview.md` that delegates to it, so the summary is reachable both as `/<plugin>:overview` in the slash menu and by asking "what does this plugin do?". The skill body is the single source of truth; the command just launches it. This keeps a real overview discoverable in-session, since the marketplace install flow only shows the one-line description, not the README.
...
4. Add an `overview` skill + `commands/overview.md` (see the Overview entry point convention above) so the plugin is self-describing in-session.
