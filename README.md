...
- **Overview entry point.** Every plugin ships an overview skill named `<plugin>-overview` (e.g. `avista-wp-releases-overview`, at `skills/<plugin>-overview/SKILL.md`) holding a comprehensive summary — what it bundles, what each skill does, the order to use them, prerequisites. It's reachable as `/<plugin>-overview` in the slash menu (typing `/avista` clusters all of them) and surfaces when the user asks "what does this plugin do?". Skill names share a **flat namespace** in the desktop app, so the name must be globally unique — do NOT use a bare generic name like `overview` (it collides with other vendors' plugins) and do NOT also ship a `commands/overview.md` (a command and a skill claiming the same `/name` collide and the invocation silently no-ops). The skill alone is the entry point. This keeps a real overview discoverable in-session, since the marketplace install flow only shows the one-line description, not the README.
...
4. Add a `<plugin>-overview` skill (see the Overview entry point convention above) so the plugin is self-describing in-session.
5. Commit with `feat(<plugin-name>): scaffold plugin`.
6. When ready, ship with `release-skill-bundle`.
