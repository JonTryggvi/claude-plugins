...
(CLAUDE.md and `.claude/rules/`), the **auto-memory** store, and **git hygiene** before a push. All
skills are *propose-before-execute* and back up before modifying.
...
| `memory-health-check` | Lints an auto-memory store — index load budget, frontmatter, wiki-links, staleness, oversized files, duplicates, sensitive and instruction-like content. | Monthly. Works on any store: Claude Code's `~/.claude/projects/<project>/memory/`, a Cowork space, or a custom `autoMemoryDirectory`. |
...
- **Memory lifecycle:** `memory-health-check` is the lightweight monthly linter; pair with the
  Anthropic `consolidate-memory` skill for the heavier quarterly reflective pass. The current spec
  it lints against — paths, the 200-line/25KB index budget, frontmatter fields, privacy categories —
  lives in `memory-health-check/references/memory-spec.md` with the date it was last verified.
